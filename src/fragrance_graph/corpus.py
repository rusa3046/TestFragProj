...
#: service layer) belong on *neither* list above, and the omission is on
#: purpose rather than an oversight this comment exists to head off.
#:
#: They are not `DERIVED_TABLES`: nothing here is computed FROM the corpus
#: — a session is a live conversation with one shopper, not a rollup of
#: claims — so there is no rebuild command that would ever refill them
#: from `data/corpus/`, and no liveness query would mean anything either.
#:
#: They are not `CURATED_INPUT_TABLES` either: that list is for
#: hand-curated, committed files that sit *alongside* the corpus as trusted
#: input (`data/curation/houses.jsonl`). A session is neither committed nor
#: curated — it is operational data generated by ordinary use of the
#: service, the same category as a web server's access log, and it holds
#: no claim, alias, or evidence that `export_corpus`/`import_corpus` would
#: ever have reason to move. It is not exported, not imported, and a
#: rebuilt database is correctly missing it entirely — not stale, simply
#: absent, and that absence needs no rebuild command because there is
#: nothing here for one to rebuild.
