...
-- `json_extract(raw_json, '$.videoId')` in SQLite; in Postgres the
-- payload is still a TEXT column, so it is cast to jsonb for the read.
-- ->> returns text and yields NULL for a missing key, which is what the
-- Reddit rows need.
UPDATE comments
   SET video_id = raw_json::jsonb ->> 'videoId'
